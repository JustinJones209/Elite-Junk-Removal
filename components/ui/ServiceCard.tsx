...
export function ServiceCard({ icon: Icon, title, description, href = "/quote" }: ServiceCardProps) {
  return (
    <div className="group h-full rounded-2xl border border-gray-100 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl">
      <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-xl bg-safety-orange/10 text-safety-orange transition-colors group-hover:bg-brand-gradient group-hover:text-white">
        <Icon className="h-7 w-7" aria-hidden="true" />
      </div>
      <h3 className="mb-2 font-heading text-xl font-700 text-navy">{title}</h3>
      <p className="mb-5 text-gray-600">{description}</p>
      <Link
        href={href}
        className="inline-flex items-center gap-1.5 font-body font-600 text-safety-orange transition-colors hover:text-safety-orange-dark focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-safety-orange"
